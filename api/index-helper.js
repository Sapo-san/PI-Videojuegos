...
const Genre = conn.models["Genre"]
const Genrevideogame = conn.models["Genrevideogame"]
    ...
    for (let index = 0; index < pfArray.length; index++) {
        switch (attribute) {
            case "platforms":
                ...
                break;
            case "genres":
                strArray.push(pfArray[index]["slug"])
                break;
            default:
...
function gameProcessor(game) {
    ...
    return {
        ...
        genres: atributeProcessor(game.genres, "genres").join(" ")
    }
}
...
async function insertGamesintoDB(promise) {
    ...
    /* genreAsociation = gameData.map((game) => {
        let associations = []
        game.genres.forEach(elem => { // elem son los ID de los generos
            associations.push({ web_id_game: game.web_id, web_id_genre: elem })
        });
    }) */
    
    
    // insertar datos con sequelize...
    return Videogame.bulkCreate(gameData).then(data => {
        // luego de insertar datos en BD
        console.log(`--- Inserción de juegos en BD exitosa ---`)

        /* Genrevideogame.bulkCreate(genreAsociation).then(() => {
            console.log(`--- Inserción de asociacion juego-genero en BD exitosa ---`)
        }) */
    })
...
async function insertGenresintoDB(promise) {
    ...
    const gameData = await promise.data.results.map((genre) => {
        return {
            web_id: genre.id,
            name: genre.slug
        }
    ...
    // insertar datos con sequelize...
    return Genre.bulkCreate(gameData)
